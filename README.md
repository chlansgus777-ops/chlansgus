...
- 재상장은 새 상장 구간입니다. 상장폐지 기록은 남고, 이전 가격은 이전 구간에 보관되며 공백을 넘어 이어 붙이지 않습니다.
  SEC 목록에서 7일 이내로 빠졌다 돌아온 종목은 데이터 누락으로 보고 이력을 그대로 잇습니다.
- 한 회사의 여러 주식 클래스가 한 번에 개명되면 클래스 접미사로 짝을 지을 수 있을 때만 이력을 연결하고, 짝이 불분명하면 연결하지 않습니다.
- 이전 버전이 이미 잘못 기록한 종목 이력(예: 개명을 상장폐지로 기록)은 이번 수정으로 자동 복구되지 않습니다. 기존 DB라면 재동기화가 필요할 수 있습니다.
