...
- 모의투자는 일봉 기준이라 장중 순서를 알 수 없습니다(같은 날 목표·손절 동시 도달 시 손절 우선, 화면에 명시).
  추천 이후 액면분할이 있으면 추천 당시의 손절·목표·최대 매수가를 분할 비율로 나눠 저장된(분할 반영) 가격과 같은 기준에서 비교합니다.
  분할 반영은 동기화가 그 분할을 적용한 뒤에만 정확합니다(분할 당일 동기화 전에는 기존 가격 기준).
- 주식분할은 실제로 실행된 날(뉴욕 날짜) 이후에만 가격에 반영합니다. 발표만 된 미래 분할은 가격을 바꾸지 않습니다.
- 가이던스 추출은 규칙 기반입니다. 손실 가이던스("loss per share of $1.10 to $1.20")는 음수로 저장하고, 손실/이익이 섞이거나
  부호를 확정할 수 없는 문장은 GUIDANCE_UNCLEAR(값 없음)로 둡니다.
- 두 무료 추정치 공급자가 10% 넘게 다르면(SEVERE) EPS 추정치를 쓰지 않고 신규 매수를 막습니다(평균 내거나 한쪽을 고르지 않음).
- 은행 전용 지표(extras)는 분기 최초 공시 값만 사용하며, 이후 정정 이력은 아직 추적하지 않습니다.
- SEC 분기 재무는 동기화가 한 번에 최대 150종목씩 나눠 수집합니다(수집 기록·실패 재시도 간격 저장). 스캔 중에는 종목마다 SEC를 호출하지 않으므로,
  첫 동기화 직후에는 재무가 아직 없는 종목이 있고, 대형주 재무 수집률이 80% 미만이면 스캐너는 "준비 안 됨"으로 표시됩니다.
- 종목 동일성은 SEC CIK로 판단합니다(티커 변경은 이력 연결, 티커 재사용은 이전 회사 데이터 분리). CIK가 없던 과거 저장분은 그대로 받아들이므로,
  종목 마스터 도입 이전에 일어난 티커 재사용은 자동으로 분리되지 않습니다. 추정치 이력은 티커 변경 후 새 티커로 다시 쌓입니다.
- 보정(Calibration)의 업종·국면별 결과는 검토용 가중치 제안일 뿐이며, 점수 계산에는 항상 하나의 운영 가중치만 씁니다.
- 장중 추천은 새 시세 없이 분석 후 20분까지만 "현재 유효"입니다. 그 뒤로는 목록 화면(캐시된 시세만 사용)에서 "현재가 재확인 필요"로 보이고,
  종목 화면이 현재가를 받아 가격 조건을 다시 확인해야 "현재 유효"가 됩니다.
- 실행 중에 `marketlens.db` 파일만 복사하지 마세요(-wal 내용이 빠져 손상될 수 있음). 백업은 `marketlens backup <경로>`를 쓰세요.
- 클래스주 표기(SEC `BRK-B` / Polygon `BRK.B`)를 아직 통일하지 않았습니다. 실제 데이터로 확인 전이며, 표기가 다르면 해당 종목에 가격이 붙지 않습니다.
- 가이던스 부호는 보수적으로 읽습니다. 손실 단어가 있는 문장은 "net loss / loss per share"가 그 숫자를 직접 가리키는 경우만 음수로 저장하고,
  그 밖에는 값 없이 GUIDANCE_UNCLEAR로 둡니다. 같은 지표의 금액이 한 문장에 둘 이상이면(GAAP·non-GAAP 등) 역시 UNCLEAR입니다.
- 재상장은 새 상장 구간입니다. 상장폐지 기록은 남고, 이전 가격은 이전 구간에 보관되며 공백을 넘어 이어 붙이지 않습니다.
  SEC 목록에서 빠졌던 기간에도 일봉이 계속 저장됐다면(계속 거래됨) 데이터 누락으로 보고 이력을 그대로 잇습니다. 거래 증거가 없으면
  7일 이내 누락만 데이터 누락으로 봅니다.
- 한 회사의 여러 주식 클래스가 한 번에 개명되면 클래스 접미사로 짝을 지을 수 있을 때만 이력을 연결합니다. 짝이 불분명하면 연결하지 않고,
  사라진 옛 티커를 상장폐지로 확정하지 않은 채 "미확정(UNRESOLVED)"으로 두어 성과 계산도 보류합니다.
- `marketlens audit-history`: 이전 버전이 남겼을 수 있는 잘못된 기록(거래가 이어진 상장폐지, 개명으로 보이는 상장폐지, CIK 없는 행,
  미확정 개명)을 찾아 보고합니다(읽기 전용, 자동 수정 없음).
- 실제 공급자 검증: GitHub Actions의 `live-verify` 워크플로(수동 실행)가 실제 API를 호출합니다. 저장소 Secrets에 키와 `SEC_USER_AGENT`
  ("이름 이메일")를 넣어야 하며, 키가 없는 공급자는 BLOCKED_BY_CREDENTIAL로 보고됩니다.
- 이전 버전이 이미 잘못 기록한 종목 이력(예: 개명을 상장폐지로 기록)은 자동 복구되지 않습니다. `marketlens audit-history`로 찾은 뒤, 필요하면 재동기화하세요.
